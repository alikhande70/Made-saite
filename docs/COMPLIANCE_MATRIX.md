...
| S11 ↑ | Sitemap scales past one file | PASS | sitemap **index** + deterministic 10,000-URL chunks; benchmarked at 10k and 50k products — worst chunk 132 ms, 1.2 MB, flat with catalogue size. Replaces a single flat sitemap that selected every active product on every request | `sitemap-service.ts`, `scripts/bench-sitemap.ts` | — | — |
| S12 ↑ | Sitemap tells the truth | PASS | only active, canonical, public URLs; every entry additionally passes `assertSitemapCoherent`, which throws on off-origin, query-carrying or private URLs. Falsified: deactivating a product removes it; 13 private prefixes each rejected | `search-visibility.test.ts` | — | — |
| S13 | `lastmod` is never synthesised | PASS | products carry real `updated_at`; categories, brands, landing and static pages carry none rather than a generated timestamp | integration test | — | — |
| S14 ↑↓ | One canonical authority | PASS | **was fourteen hand-built canonicals across page files.** Persian slugs double-encoded when a caller passed an already-encoded slug, giving two canonicals for one product. Now one `canonicalPath` used everywhere | `domain/search-visibility.ts` | — | — |
| S15 ↑↓ | Breadcrumb JSON-LD matches the visible trail | PASS | **they had drifted**: the JSON-LD omitted the «دسته‌بندی‌ها» level the page rendered, and omitted the product itself. Both now derive from one array; an E2E test compares them element by element | `products/[slug]/page.tsx` | — | — |
| S16 | Product structured data omits what it lacks | PASS | `image` omitted entirely when a product has none, rather than emitting an empty array | `products/[slug]/page.tsx` | — | — |
| S17 | Organization / WebSite data | PASS | built from real store-profile rows; contact block omitted when the shop has filled in neither phone nor email | `lib/json-ld.ts` | — | — |
| S18 | Change notification is async and retryable | PASS | DB outbox drained by the existing sweeper; dedupe by partial unique index, exponential backoff, status-aware retry, `SKIP LOCKED` for concurrent replicas. Falsified: an adapter that throws returns 0 rather than failing the product save | `outbox.ts`, integration tests | — | — |
| S19 | IndexNow follows the published protocol | PASS | key 8–128 `[A-Za-z0-9-]`, key file at `/{key}.txt`, batch POST with `{host, key, keyLocation, urlList}`, 202 treated as success. Asserted against a captured request body | `indexnow-adapter.ts` | — | — |
| S20 | Submission secrets never leak | PASS | key read from env, served only at its own path, asserted absent from failure messages; all submission is server-side | unit + integration | — | — |
| S21 | SEO health is attributable, not a vanity metric | PASS | deterministic score; every deduction maps to a counted issue with a per-code cap. Falsified: removing a product's category raises exactly that issue's count and lowers the score | `seo-health.ts` | — | — |
| S22 | Admin search-visibility surface | PASS | groups, engine status, queue, failures and issues; behind `adminRoute`; anonymous access 401/redirect (E2E) | `/admin/search-visibility` | — | — |
| S23 | Faceted crawl explosion controlled | PASS | one indexable URL per listing surface; filtered, sorted and paginated variants `noindex, follow` and absent from the sitemap; `follow` never withdrawn | unit + E2E | — | — |
| S24 | No mass thin-page generation | PASS | a landing page needs an active category, an active model, a **positive** fitment row and a configurable product threshold. Falsified: 500 products with no positive fitment is still not indexable | `search-visibility.test.ts` | — | — |
| S25 | Search Console / Bing verification | **UNKNOWN** | requires a real domain; runbook written, no credentials committed | `docs/SEARCH_VISIBILITY.md` §9 | **P1** | after domain |
| S26 | IndexNow accepted by a live engine | **UNKNOWN** | the adapter is asserted against a captured request, never against `api.indexnow.org`. Needs the key file reachable at a real HTTPS domain | — | **P1** | after domain |
| S27 | Real indexing coverage | **UNKNOWN** | whether engines actually index what we advertise is only observable in Search Console after deployment | — | **P1** | after domain |
| S28 | Slug-change redirects | **OPEN** | a changed slug submits the old URL for re-crawl, but there is no redirect table, so it 404s rather than 301-ing. Deliberately not built: untested machinery for a case that has not occurred | — | P2 | when slugs change |
...
| S29 ↑↓ | Root `force-dynamic` reassessed | PARTIAL | the earlier audit called it a blunt instrument. It is **load-bearing**: `SiteHeader` reads the session cookie and cart count on every page, so the root layout genuinely cannot be static and removing the directive would change nothing. Route-by-route classification recorded in `docs/SEARCH_VISIBILITY.md` §12. Two redundant `getStoreProfile()` queries per render removed via per-request memoisation instead. Making the catalogue cacheable needs the cart badge moved client-side — a commerce-truth trade, out of scope here | `settings-service.ts` | P2 | separate phase |
